...
try {
  ...
  function initQuestions(questionObj) {
    ...
    // $(".question").html('');//清空
    $(".question").html(
    "<div class='content-block-title'>"+(_index + 1) +" .【"+result+"】&nbsp; "+ questionObj.question+"</div>"+
      ...
      $.each(answers, function (i, val) {
        html +="<li>"+
                "<label class='label-checkbox item-content'>"+
                  "<input type='radio' name='my-radio' id='"+val.id+"'>"+
                  "<div class='item-media'>"+
                    "<i class='icon icon-form-checkbox'></i>"+
                  "</div>"+
                  "<div class='item-inner'>"+
                    "<div class='item-text'>"+
                      "<span>" + M[i] + ".</span>"+ val.answerDesc+
                    "</div>"+
                  "</div>"+
                "</label>"+
              "</li>";
...
//返回
  ...
  function initBtn(index) {
    ...
    if (index < _maxIndex - 1) {
      $(".button_box").html("<a class='button button-big button-fill' onclick='_next("+ index + ")'>下一题</a>").addClass("nextBtn");
    }
    if (index == _maxIndex - 1) {
      $(".button_box").html("<a class='button button-big button-fill' onclick='save("+ index + ")'>交卷</a>").addClass("subBtn");
    }
  }
  ...
  function save(_index) {
    ...
    $(".button-fill").removeAttr("onclick");
    $.ajax({
  ...
  function changeTime() {
    ...
    $(".time").html( "剩余时间:<span>"+minutes + " 分钟" + leave3 + "秒</span>");
    if (_times <= 0) {
